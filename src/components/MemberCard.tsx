...
const UserLink = ({
  ...
  if (image) {
    if (image.startsWith('/')) {
      // This image is in the static folder and should be referenced with `useBaseUrl` hook
      userImage = <img src={useBaseUrl(image)} alt={name} title={name} />;
    } else {
      userImage = <img src={image} alt={name} title={name} />;
    }
  } else {
    // If `image` does not exist, substitute for Default image
    userImage = <img src={useBaseUrl('img/alumni/Default.svg')} alt={name} title={name} />;
  }
